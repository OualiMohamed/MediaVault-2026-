...
const metadata = computed(() => {
    ...
    if (type.value === 'movie') {
        if (d.original_title && d.original_title !== item.value.title) {
            rows.push({ label: 'Original Title', value: d.original_title })
        }
...
const tags = computed(() => {
    ...
    // Add this after the format tag
    if (type.value === 'movie' && d.video_tier) {
        list.push({
            label: `${d.video_tier}`,
