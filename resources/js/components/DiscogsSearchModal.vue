...
async function doSearch() {
    ...
    try {
        const { data } = await api.post('/discogs/search', { query: query.value })
...
async function selectItem(item) {
    ...
    try {
        const { data } = await api.post('/discogs/details', { discogs_id: item.id })
            ...
            <div
                ...
                <div class="flex items-center justify-between px-5 py-4 border-b border-vault-700 flex-shrink-0">
                    <div class="flex items-center gap-2.5">
                        <div class="w-8 h-8 rounded-lg bg-violet-500/15 flex items-center justify-center">🎵</div>
                        <h2 class="text-white font-semibold">Search Discogs</h2>
                    </div>
                ...
                <div class="px-5 py-3 border-b border-vault-700 flex-shrink-0">
                    <div class="relative">
                        ...
                        <input ref="searchInput" v-model="query" type="text" @input="onSearchInput"
                            class="w-full pl-10 pr-10 py-2.5 bg-vault-700 border border-vault-600 rounded-xl text-white placeholder-vault-400 focus:outline-none focus:ring-2 focus:ring-violet-500/50 text-sm"
                            placeholder="Search for an album or artist..." autocomplete="off" />
                    ...
                    <p class="text-vault-500 text-xs mt-1.5">Powered by Discogs — great for vinyl, CDs & cassettes</p>
                </div>
                    ...
                    <div v-else>
                        ...
                        <div class="space-y-1">
                            <button v-for="item in results" :key="item.id" @click="selectItem(item)"
                                ...
                                <div
                                    class="w-14 h-14 rounded-lg bg-vault-700 overflow-hidden flex-shrink-0 border border-vault-600 aspect-square">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p
                                        class="text-white text-sm font-medium truncate group-hover:text-violet-400 transition-colors">
                                        {{ item.title }}</p>
                                    <div class="flex items-center gap-2 mt-0.5">
                                        <span v-if="item.artist" class="text-vault-400 text-xs truncate">{{ item.artist
                                            }}</span>
                                        <span v-if="item.artist && (item.year || item.format)"
                                            class="text-vault-600">&middot;</span>
                                        <span v-if="item.format" class="text-violet-400 text-xs truncate">{{ item.format
                                            }}</span>
                                        <span v-if="item.format && item.year" class="text-vault-600">&middot;</span>
                                        <span v-if="item.year" class="text-vault-500 text-xs">{{ item.year }}</span>
                                    </div>
                                </div>
                                <div v-if="fetchingDetails"
                                    class="w-4 h-4 border-2 border-violet-500 border-t-transparent rounded-full animate-spin flex-shrink-0">
                                </div>
                                <svg v-else
                                    class="w-4 h-4 text-vault-600 group-hover:text-violet-400 transition-colors flex-shrink-0"
                        ...
                        <div v-if="query.length < 2" class="text-center py-12">
                            <div
                                ...
                                🎵</div>
                            <p class="text-vault-400 text-sm">Search for an album to auto-fill details and cover</p>
                        </div>
                    </div>
                </div>
