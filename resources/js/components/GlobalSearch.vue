...
<template>
    <div class="relative flex-1 max-w-xs sm:max-w-sm lg:max-w-md">
        <div class="relative">
            ...
            <button v-else-if="query" @click="close"
                class="absolute inset-y-0 right-0 pr-3 flex items-center text-vault-500 hover:text-white">
            ...
            <div v-if="isOpen && (results.length > 0 || loading)"
                ...
                <template v-else>
                    <div class="px-4 py-2 border-b border-vault-700 flex items-center justify-between bg-vault-900/50">
                        <span class="text-vault-400 text-xs font-medium">{{ results.length }} result{{ results.length
                            !== 1 ? 's' : '' }}</span>
                        <button @click="close"
